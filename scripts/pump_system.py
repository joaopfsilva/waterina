...
def turn_off_pumps(pin_id):
  try:
    ...
    print(json.dumps({'result': 'NOK', 'message': e}))
...
def water_plant(plant_id):
  ...
  if pin_id == None:
    print(json.dumps({'result': 'NOK', 'message': 'Invalid plant id: {}'.format(plant_id)}))
  else:
    try:
      ...
      GPIO.output(pin_id, GPIO.LOW)
      print(json.dumps({'result': 'OK', 'message': 'Plant {} was successfully watered'.format(plant_id)}))
    except:
      print(json.dumps({'result': 'NOK', 'message': 'Something went wrong'}))
    finally:
